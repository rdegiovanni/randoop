...
public class SequenceParser {
  ...
  /**
   * Assumes the code contains simple statements, e.g., parameters to method and constructor calls
  ...
  public static Sequence codeToSequence(String code, List<String> imports, String forClass)
      throws SequenceParseException {
    return codeToSequence(code, imports, forClass, false, new ArrayList<>()).a;
  }

  /**
   * Returns Randoop sequence corresponding to input sequence, and the indices of original
   * statements in the Randoop sequence. Note that original statements may be modified, e.g.,
   * constants passed as arguments to a method may be replaced with variables holding the constant
   * value
   */
  public static Pair<Sequence, Boolean> codeToSequence(
      String code,
      List<String> imports,
      String forClass,
      boolean addPackageDeclaration,
    ...
    List<String> formattedStatements =
        statementsToRandoopStatement(augmentedCode, originalIndices, indexToParameterTypes);
  ...
  private static Sequence addParameterTypes(
      Sequence seq, Map<Integer, NodeList<Type>> indexToParameterTypes) {
      ...
      if (indexToParameterTypes.containsKey(i)) {
        if (!statement.isConstructorCall()) {
          throw new IllegalArgumentException(
              "Expected constructor statement but received " + statement.getOperation().getName());
        }
  ...
  private static String augmentCode(
      String code, List<String> imports, String forClass, boolean addPackageDeclaration) {
  ...
  private static List<String> statementsToRandoopStatement(
      String code,
      List<Integer> originalIndices,
  ...
  private static List<String> parseStatement(
    ...
    if (node instanceof VariableDeclarationExpr) {
      ...
      throw new IllegalArgumentException(
          "Encountered unrecognized statement: " + statement.toString());
    }
  ...
  private static void parseVariableDec(
  ...
  private static void parseAssignmentExpression(
      AssignExpr expr,
      List<String> randoopStatements,
      Map<Integer, NodeList<Type>> indexToParameterTypes)
      throws ClassNotFoundException, NoSuchFieldException, SecurityException {
    ...
    if (target instanceof FieldAccessExpr) {
      randoopStatement.append(
          getNextVar() + " = " + getFieldAccessStatement((FieldAccessExpr) target, false));
    }
    ...
    //            randoopStatement.append(getNextVar() + " = " + RANDOOP_ARRAY_ELEMENT_SET + " :
    // <set>" + targetExprType.describe() + "[] : ");
    //        }
    else {
    ...
    String valueAssignmentStatement =
        parseAssignmentValue(
            target.calculateResolvedType(), value, randoopStatements, indexToParameterTypes);
  ...
  private static String parseAssignmentValue(
    ...
    if (valueExpr instanceof ObjectCreationExpr) {
      ...
      //
      //			valueAsRandoopStatement.append(constDec.getQualifiedSignature().replaceAll("<E>",
      // "").replaceAll("<T>", "")+" : ");
      //
      //			List<ResolvedParameterDeclaration> argTypes = new
      // ArrayList<ResolvedParameterDeclaration>();
      //
      //			for (int i=0; i<constDec.getNumberOfParams(); i++) {
      //				argTypes.add(constDec.getParam(i));
      //			}
      //
      //			valueAsRandoopStatement.append(handleMethodArguments(constr.getArguments(), argTypes,
      // randoopStatements,
      //                indexToParameterTypes));
  ...
  private static void parseMethodCall(
    ...
    randoopStatement.append(
        getMethodCallStatement(methodCall, randoopStatements, indexToParameterTypes));

    randoopStatements.add(randoopStatement.toString());
  }

  private static String getMethodCallStatement(
      MethodCallExpr methodCall,
      List<String> randoopStatements,
      Map<Integer, NodeList<Type>> indexToParameterTypes)
      throws NoSuchFieldException, ClassNotFoundException {
  ...
  private static String handleMethodArguments(
    ...
    for (Expression expr : arguments) {
      ...
      if (expr instanceof NameExpr) {
        ...
        if (expr instanceof NameExpr) {
          ...
          //                        ResolvedType type = expr.calculateResolvedType();
          //                        addedStatements.add(nextVar + " = " +
          // getNonReceiverStatement(expr, type));
          //                    }
  ...
  private static String getArrayCreationStatement(
      String arrayCreateExpr,
      ArrayInitializerExpr initExpr,
    ...
    if (arrayCreateExpr.equals("null")) {
      ...
      String content =
          arrayCreateExpr.substring(arrayCreateExpr.toString().indexOf("new ") + "new ".length());

      long count = content.chars().filter(ch -> ch == '[').count();

      if (count > 1) {
        throw new IllegalArgumentException(
            "Currently do not support parsing of multi-dimensional arrays");
      }
      ...
      if (lengthStr.length() > 0) {
        ...
        if (initExpr == null) {
          throw new IllegalArgumentException(
              "Missing array initialization at " + arrayCreateExpr.toString());
        }
      ...
      if (initExpr == null) {
        ...
        if (actualExpr instanceof NameExpr) {
          ...
          throw new RuntimeException(
              "Unexpected array initialization type " + actualExpr.toString());
        }
      }
  ...
  private static String getFieldAccessStatement(FieldAccessExpr expr, boolean isGet)
    ...
    String statement =
        (isGet ? RANDOOP_FIELD_GET : RANDOOP_FIELD_SET)
            + " : "
            + className
            + ".<"
            + (isGet ? "get" : "set")
  ...
  private static String getNonReceiverStatement(Expression expr, ResolvedType type) {
    ...
    if ((expr instanceof LongLiteralExpr
            || (expr instanceof UnaryExpr
                && ((UnaryExpr) expr).getExpression() instanceof LongLiteralExpr))
        && exprStr.endsWith("L")) {
      exprStr = exprStr.substring(0, exprStr.length() - 1);
    } else if (expr instanceof CharLiteralExpr) {
      // String is 'c' so we need second entry
      char c = exprStr.charAt(1);
      exprStr = Integer.toString(c);
    } else if (expr instanceof DoubleLiteralExpr
        || ((expr instanceof UnaryExpr
                && ((UnaryExpr) expr).getExpression() instanceof DoubleLiteralExpr)
  ...
  private static String getConstructorCallStatement(
      ObjectCreationExpr constructorCallExpr,
      List<String> randoopStatements,
      Map<Integer, NodeList<Type>> indexToParameterTypes)
      throws NoSuchFieldException, ClassNotFoundException {
    ...
    constructorCallStmt.append(
        handleMethodArguments(
            constructorCallExpr.getArguments(),
            argTypes,
            randoopStatements,
            indexToParameterTypes));
  ...
  //				if ( !
  // methodArg.getName().asString().equals(DiffAssertionsGenerator.FIELD_UTIL_METHOD_NAME)) {
